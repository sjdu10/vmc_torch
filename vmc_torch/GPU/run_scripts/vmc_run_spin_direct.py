"""GPU VMC for spin-1/2 Heisenberg with direct-sample-as-proposal MCMC.

Same setup as vmc_run_spin.py but swaps the sampler for
`DirectProposalMCMCSamplerSpinGPU`. Direct sampling runs eagerly per walker
on CPU (Python loop over B), so we use a much smaller batch_size than the
exchange-sampler script.

Run:
    torchrun --nproc_per_node=<N> vmc_run_spin_direct.py
    torchrun --nproc_per_node=1 vmc_run_spin_direct.py   # single GPU
"""
import os
...
# Smaller batch_size than vmc_run_spin.py because direct_sample is a per-walker
# Python loop on CPU (~30ms / walker for D=2 4x4).
vmc_cfg = VMCConfig(
    batch_size=512,
    ns_per_rank=512,
    grad_batch_size=256,
...
def main():
    ...
    try:
        ...
        # Direct sampling parameters (None / 0 for chi_m = chi_m=0 approx)
        chi_s_sample = 4
        chi_m_sample = None
        ...
        output_dir = (
            ...
            f"J={J}/D={D}/chi={chi}/direct_mcmc/"
        )
        os.makedirs(output_dir, exist_ok=True)
        model_name = model._get_name() + "_direct"
        N_params = sum(
        ...
        if rank == 0:
            ...
            print(
                f"Sampler: DirectProposalMCMCSamplerSpinGPU "
                f"(chi_s={chi_s_sample}, chi_m={chi_m_sample})"
            )
        ...
        # ========== VMC driver ==========
        # Use rank-dependent base_seed so all walkers across ranks are unique.
        sampler_base_seed = 100000 * (rank + 1)
        vmc = VMC_GPU(
            sampler=DirectProposalMCMCSamplerSpinGPU(
                chi_s=chi_s_sample,
                chi_m=chi_m_sample,
                total_sz=0,
                base_seed=sampler_base_seed,
            ),
