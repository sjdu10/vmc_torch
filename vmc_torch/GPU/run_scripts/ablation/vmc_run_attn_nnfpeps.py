"""GPU VMC with Attention-Geometric NN-fPEPS backflow model — 8x8 system.

Run:
    torchrun --nproc_per_node=1 run_scripts/vmc_run_attn_nnfpeps.py
    torchrun --nproc_per_node=2 run_scripts/vmc_run_attn_nnfpeps.py
"""
import torch
...
def main():
    ...
    try:
        ...
        # Attention backflow hyperparameters
        nn_eta = 1.0
        d_model = 16
        n_heads = 4
        n_layers = 1
        dim_feedforward = 4*d_model
        radius = 2  # None=global, int=local Chebyshev distance
        head_hidden_dim = 64  # hidden layer in output heads
        ...
        model = Attention_Geometric_fPEPS_GPU(
            tn=peps,
            max_bond=chi,
            nn_eta=nn_eta,
            d_model=d_model,
        ...
        output_dir = (
            ...
            f"{model._get_name()}_r={radius}/chi={chi}/"
        )
        import os
        ...
        if rank == 0:
            ...
            radius_str = (
                f"radius={radius}" if radius is not None
                else "global"
            )
            print(
                f"nn_eta={nn_eta}, d_model={d_model}, "
                f"n_heads={n_heads}, n_layers={n_layers}, "
                f"dim_ff={dim_feedforward}, {radius_str}, "
                f"head_hidden={head_hidden_dim}"
            )
            print(
                f"backbone_dtype={nnbackbone_dtype}, "
                f"TN dtype={dtype}"
            )
            print(
