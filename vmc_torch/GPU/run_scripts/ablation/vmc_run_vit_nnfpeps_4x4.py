"""GPU VMC with ViT-Geometric NN-fPEPS backflow model — 4x4 system.

Run:
    torchrun --nproc_per_node=1 run_scripts/vmc_run_vit_nnfpeps_4x4.py
    torchrun --nproc_per_node=2 run_scripts/vmc_run_vit_nnfpeps_4x4.py
"""
import torch
...
def main():
    ...
    try:
        rank, world_size, device = setup_distributed(cuda_rank=0)
        ...
        model_name = (
            model._get_name()
            + f'_depth={vit_layers}_heads={n_heads}'
            + f'_hidden={hidden_dim}_ffmult={ff_mult}'
        )
        output_dir = (
        ...
        if rank == 0:
            ...
            print(
                ...
                f"hidden={hidden_dim}, "
                f"layers={vit_layers}, n_heads={n_heads}, "
                f"ff_mult={ff_mult}"
            )
            print(
        ...
        system_str = (
            ...
            f'nn_eta={nn_eta}, embed={embed_dim}, '
            f'layers={vit_layers}, n_heads={n_heads}, '
            f'hidden={hidden_dim}, ff_mult={ff_mult}, '
            f'backbone_dtype={nnbackbone_dtype}, '
            f'TN dtype={dtype}'
        )
