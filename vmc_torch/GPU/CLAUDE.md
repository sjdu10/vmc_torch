...
All commands should be run from the `vmc_torch/GPU` directory. The scripts import via `vmc_torch.GPU.*`.
...
**GPU** (`vmc_torch/GPU/`): All ranks sample independently (no master-worker dispatch). Uses `torch.distributed.all_reduce` for aggregation. Each rank owns `B` walkers and accumulates `Ns_per_rank` samples across multiple sweeps.
