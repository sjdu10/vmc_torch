...
class Variational_State:
    def __init__(
        self, vstate_func, sampler=None, hi=None, dtype=torch.float32, iprint=0
    ):
    ...
    def amplitude_grad(self, x, retain_graph=False):
        ...
        if isinstance(
            self.vstate_func, vmc_torch.experiment.tn_model.fTNModel_reuse
        ) or isinstance(
            self.vstate_func, vmc_torch.experiment.tn_model.PEPS_model_reuse
        ) or isinstance(
            ...
            try:
                if self.vstate_func.debug:
                    ...
                    vec_log_grad = get_safe_ratio(
                        self.vstate_func.params_grad_to_vec().detach(), amp.detach()
                    )
                else:
            ...
            if DEBUG:
                ...
                if (
                    torch.norm(vec_log_grad - autodiff_grad) / torch.norm(autodiff_grad)
                    > 1e-3
                ):
                    print(
                        f"Rank {RANK} amplitude: {amp}, rel grad err: {torch.norm(vec_log_grad - autodiff_grad) / torch.norm(autodiff_grad)}"
                    )
    ...
    def full_hi_logamp_grad_matrix(self, grad=True):
        ...
        parameter_logamp_grad = torch.zeros(
            (self.Np, local_configs.shape[0]), dtype=self.dtype
        )
    ...
    def full_hi_expect(self, op):
        ...
        """
        self.full_hi_logamp_grad_matrix(
            grad=False
        )  # cache the psi_i(n)/psi(n) and psi(n)
        ...
        stats_dict = {
            "mean": float(expect_op.detach().numpy())
    ...
    def full_hi_expect_and_grad(self, op):
        ...
        if RANK == 0:
            ...
            expect_amp_grad = (
                torch.sum(weights.unsqueeze(0) * parameter_logamp_grad, dim=1)
                / norm_sqr
            )  # <dlnpsi/dp>
            expect_E_loc_amp_grad = (
                torch.sum(
                    weights.unsqueeze(0)
                    * E_loc_vec.unsqueeze(0)
                    * parameter_logamp_grad,
                    dim=1,
                )
                / norm_sqr
            )  # <E_loc dlnpsi/dp>
    ...
    def expect_and_grad(self, op, full_hi=False, message_tag=None):
        ...
        if self.equal_partition:
            ...
            t0 = MPI.Wtime()
            op_expect, op_grad, op_var, op_err = self.collect_samples_w_grad(
                op, chain_length=chain_length
            )
            t1 = MPI.Wtime()
            ...
            assert message_tag is not None, (
                "Message tag must be provided for eager sampling!"
            )
            # use MPI for sampling, but sampler may have different number of samples per rank
            t0 = MPI.Wtime()
            op_expect, op_grad, op_var, op_err = self.collect_samples_w_grad_eager(
                op, message_tag=message_tag
            )
            t1 = MPI.Wtime()
        ...
        assert self.logamp_grad_matrix is not None, (
            "Logamp gradient matrix must be computed during sampling!"
        )
    ...
    def expect(self, op, full_hi=False, message_tag=None, vec_op=False, pgbar=True):
        ...
        Args:
            op (operator object): The operator for which the expectation value and gradient are calculated.
            full_hi (bool): Whether to use the full Hilbert space expectation value and gradient calculation.
        ...
        if self.equal_partition:
            ...
            # use MPI for sampling
            t0 = MPI.Wtime()
            op_expect, op_var, op_err = self.collect_samples(
                op, chain_length=chain_length, vec_op=vec_op
            )
            t1 = MPI.Wtime()
            self.equal_sampling_time = t1 - t0
            ...
            if message_tag is None:
                ...
                if RANK == 0:
                    print(
                        "MPI message tag needed for proper synchronization in eager sampling. Default message tag is set to 0."
                    )

            # use MPI for sampling, but sampler may have different number of samples per rank
            t0 = MPI.Wtime()
            op_expect, op_var, op_err = self.collect_samples_eager(
                op, message_tag=message_tag, vec_op=vec_op, pgbar=pgbar
            )
            t1 = MPI.Wtime()
            self.eager_sampling_time = t1 - t0
    ...
    def collect_samples(self, op, chain_length=1, vec_op=False, pgbar=True):
        ...
        local_samples = self.sampler.sample(
            op=op, vstate=vstate, chain_length=chain_length, vec_op=vec_op, pgbar=pgbar
        )
        self.clear_cache()
        ...
        op_expect = (
            COMM.allreduce(local_op_loc_sum, op=MPI.SUM) / self.Ns
        )  # op_expect is seen by all ranks
        t01 = MPI.Wtime()
        ...
        if not vec_op:
            ...
            # (n_i - 1)* s^2_i
            local_op_loc_sqrd_sum = (chain_length - 1) * local_op_var + chain_length * (
                local_op_loc_mean - op_expect
            ) ** 2
            ...
            chain_means = COMM.gather(
                chain_means_loc, root=0
            )  # [[m_11, m_12], [m_21, m_22], ..., [m_q1, m_q2]] where q is the number of ranks
        else:
            op_var = None
        ...
        if RANK == 0:
            print(
                "RANK{}, sample size: {}, chain length per rank: {}".format(
                    RANK, self.Ns, chain_length
                )
            )
            ...
            op_var = op_var / self.Ns if op_var is not None else None
            ...
            if not vec_op:
                ...
                V_hat = (
                    sub_chain_length - 1
                ) / sub_chain_length * W + B / sub_chain_length  # + mean_correction
                R_hat = np.sqrt(V_hat / W)
                print("R_hat: {}".format(R_hat))
                print("V_hat:{}, OP_var:{}".format(V_hat, op_var))

            op_mean_err = (
                np.sqrt(op_var / self.Ns) if op_var is not None else None
    ...
    def collect_samples_eager(self, op, message_tag=None, vec_op=False, pgbar=True):
        ...
        t_sample_start = MPI.Wtime()
        local_samples = self.sampler.sample_eager(
            op=op, vstate=vstate, message_tag=message_tag, vec_op=vec_op, pgbar=pgbar
        )
        self.clear_cache()
        t_sample_end = MPI.Wtime()
        # if DEBUG:
        ...
        # Compute the op expectation value on all ranks
        op_expect = (
            COMM.allreduce(local_op_loc_sum, op=MPI.SUM) / total_sample_Ns
        )  # op_expect is seen by all ranks
        t01 = MPI.Wtime()
        ...
        if not vec_op:
            ...
            if n_local_samples > 0:
                ...
                local_W_var = (n_local_samples - 1) * local_op_var + n_local_samples * (
                    local_op_loc_mean - op_expect
                ) ** 2
            else:
                local_op_loc_mean = 0.0
                local_W_var = 0.0
        ...
        if RANK == 0:
            ...
            # Standard error of the mean value of the local op
            op_var = op_var / total_sample_Ns if op_var is not None else None
            op_mean_err = (
                np.sqrt(op_var / total_sample_Ns) if op_var is not None else None
            )
    ...
    def collect_samples_w_grad(self, op, chain_length=1):
        ...
        # a tuple of (op_loc_sum, logpsi_sigma_grad_sum, op_logpsi_sigma_grad_product_sum, op_loc_var, logpsi_sigma_grad_mat, W_loc, chain_means_loc)
        t_sample_start = MPI.Wtime()
        local_samples = self.sampler.sample_w_grad(
            op=op, vstate=vstate, chain_length=chain_length
        )
        self.clear_cache()
        t_sample_end = MPI.Wtime()
        # if DEBUG:
        ...
        op_logamp_grad_product_sum = COMM.reduce(
            local_op_logamp_grad_product_sum, op=MPI.SUM, root=0
        )
        t02 = MPI.Wtime()
    ...
    def collect_samples_w_grad_eager(self, op, message_tag=None):
        ...
        t_sample_start = MPI.Wtime()
        local_samples = self.sampler.sample_w_grad_eager(
            op=op, vstate=vstate, message_tag=message_tag
        )
        self.clear_cache()
        t_sample_end = MPI.Wtime()
        # if DEBUG:
        ...
        mean_logamp_grad = (
            COMM.allreduce(local_logamp_grad, op=MPI.SUM) / total_sample_Ns
        )
        t01 = MPI.Wtime()
    ...
    def collect_SWO_dataset_eager(self, op, message_tag=None):
        ...
        t_sample_start = MPI.Wtime()
        local_dataset = self.sampler.sample_SWO_dataset_eager(
            vstate=vstate, op=op, message_tag=message_tag
        )
        self.clear_cache()
        t_sample_end = MPI.Wtime()
    ...
    def collect_SWO_state_fitting_dataset_eager(
        self, target_state, message_tag=None, compute_energy=False, op=None
    ):
        ...
        t_sample_start = MPI.Wtime()
        local_dataset = self.sampler.sample_SWO_state_fitting_dataset_eager(
            vstate=vstate,
