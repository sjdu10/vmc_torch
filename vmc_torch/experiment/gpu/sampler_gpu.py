import os
import numpy as np
# avoid using MPI, use batched operations instead
...
from vmc_torch.global_var import DEBUG, set_debug, TIME_PROFILING, TAG_OFFSET
...
class Sampler(AbstractSampler):
    ...
    def __init__(self, hi, graph, N_samples=2**8, burn_in_steps=100, reset_chain=False, random_edge=False, subchain_length=None, equal_partition=True, dtype=torch.float32, batchsize=20):
    ...
    def sample(self, vstate, op, chain_length=1):
        ...
        for chain_step in range(chain_length):
            # sample the next configuration
            psi_sigmas = torch.zeros(self.batchsize, dtype=torch.int)
            # when there is zero in psi_sigmas, we need to sample again
            while torch.any(psi_sigmas == 0):
                # We need to make sure that the amplitude is not zero
                sigmas, psi_sigmas = self._sample_next(vstate)
                    
            n += 1
        
            # compute local energy and amplitude gradient
            psi_sigmas = vstate.amplitude(sigmas)
            # compute the connected non-zero operator matrix elements <eta|O|sigma>
            etas, O_etasigma = op.get_conn(sigmas) # Non-zero matrix elements and corresponding configurations
            psi_etas = vstate.amplitude(etas)

            # convert torch tensors to numpy arrays
            psi_sigmas = psi_sigmas.cpu().detach().numpy() # shape (batchsize, N_site)
            psi_etas = psi_etas.cpu().detach().numpy() # shape (batchsize, n_connected, N_site)

            # compute the local operator
            op_loc = np.sum(O_etasigma * (psi_etas / psi_sigmas), axis=-1)
