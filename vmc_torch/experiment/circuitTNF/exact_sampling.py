...
for max_bond in max_bonds:
    for tau in [0.0]:
        for depth in depths:
            ...
            E = ((state_vec@np.matmul(ham_matrix, state_vec))/(state_vec @ state_vec)).real
            print(f'Variational state energy: {E}, tau={tau}, max_bond={max_bond}')
            # collect_data_dict = {(depth, tau, max_bond): E}
            collect_data_dict = {f'L{L}_D{D}_depth={depth}_tau={tau}_maxbond={max_bond}': (E, np.float64(su_energy), np.float64(exact_energy))}
