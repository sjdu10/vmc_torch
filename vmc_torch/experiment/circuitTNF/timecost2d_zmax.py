    ...
    for max_bond in max_bonds:
        for tau in [0.04]:
            for depth in depths:
                ...
                collect_data_dict = {
                    f"Lx{Lx}_Ly{Ly}_D{D}_depth={depth}_tau={tau}_maxbond={max_bond}": average_time
                }
                
                
                # add to existing data file or create a new one
                data_file = f"./data/circuitTNF2d_heis_Lx{Lx}_Ly{Ly}_D{D}_exact_sampling_time_cost_zmax.json"
