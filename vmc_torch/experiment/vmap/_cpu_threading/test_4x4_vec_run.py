"""
Test script: verify run_sampling_phase_vec (memory-optimized, O_loc inline)
produces correct VMC optimization on 4x4 lattice.

Run with:
    mpirun -np 4 /home/sijingdu/TNVMC/VMC_code/clean_symmray/bin/python test_4x4_vec_run.py
"""
import os
...
params_path = (
    f'{pwd}/{Lx}x{Ly}/t={t}_U={U}/N={N_f}/Z2/D={D}/'
)
params = pickle.load(
...
scheduler = DecayScheduler(
    init_lr=learning_rate, decay_rate=0.9,
    patience=50, min_lr=1e-2,
)
...
torch.nn.utils.vector_to_parameters(
    curr_params, fpeps_model.parameters()
)
COMM.Barrier()

get_grads = partial(
    compute_grads, vectorize=True, vmap_grad=True,
    batch_size=B_grad, verbose=False,
)
...
for svmc in range(init_step, vmc_steps + init_step):
    ...
    valid_energies = [
        e for e in all_energies_list if e.size > 0
    ]
    ...
    if RANK == 0:
        ...
        print(
            f"Step {svmc:3d} | E/site: {e_per_site:.6f} "
            f"+/- {err:.6f} | N={total_samples} | "
            f"T={step_time:.1f}s"
        )
...
if RANK == 0:
    print(f"\n{'='*50}")
    print(f"VERIFICATION (4x4, vec variant)")
    print(f"{'='*50}")
    e_first = energy_history[0]
    e_last = energy_history[-1]
    print(f"First E/site: {e_first:.6f}")
    print(f"Last E/site:  {e_last:.6f}")
    print(f"Min E/site:   {min(energy_history):.6f}")
    print(f"Delta: {e_last - e_first:.6f}")
    print(f"Avg time/step: {np.mean(step_times):.1f}s")
