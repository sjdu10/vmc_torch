...
# ==============================================================================
from vmc_torch.experiment.vmap.vmap_utils import compute_grads_vec, random_initial_config
...
from vmc_torch.experiment.vmap.vmap_modules import run_sampling_phase_vec, distributed_minres_solver
...
# Load Checkpoint
file_path = f'{params_path}/{fpeps_model._get_name()}_vec/chi={chi}/'
...
# Grad Function — use compute_grads_vec (vec variant)
get_grads = partial(compute_grads_vec, vectorize=True, vmap_grad=True, batch_size=B_grad, verbose=False)
...
for svmc in range(init_step, vmc_steps + init_step):
    ...
    # --- Step 1: Sampling Phase (vec variant) ---
