...
# )
model = fTN_backflow_attn_Tensorwise_Model_v2(
    peps,
    max_bond=chi,
    embedding_dim=16,
    attention_heads=4,
...
# sampler = MetropolisExchangeSamplerSpinful_2D_reusable(H.hilbert, graph, N_samples=N_samples, burn_in_steps=1, reset_chain=False, random_edge=False, equal_partition=True, dtype=dtype, hopping_rate=0.25)
sampler.debug = False
if N_f == int(Lx*Ly):
